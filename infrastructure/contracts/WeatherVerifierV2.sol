    ...
    function verifyProof(
        ...
        assembly {
            ...
            function reposition_challenge_len_data() -> ret0, ret1, ret2 {
                let fsm_challenges := mload(0xc0)
                let challenge_len_ptr := 0xe0
            ...
            let theta_mptr := 0x0
            {
                ...
                theta_mptr := add(challenge_mptr, mload(0xa0))
                ...
                let num_evals := mul(0x20, mload(0x80))
                ...
                if mload(add(vk_mptr, 0x01a0)) {
                    ...
                    let cptr := add(instances.offset, mul(mload(add(vk_mptr, 0x01c0)), 0x20))
            ...
            {
                let k := mload(add(vk_mptr, 0x0100))
                let x := mload(add(theta_mptr, 0x80))
                let x_n := x
                for
                    { let idx := 0 }
                ...
                mptr := x_n_mptr
                let l_i_common := mulmod(x_n_minus_1, mload(add(vk_mptr, 0x0120)),R)
                for
                    { let pow_of_omega := mload(add(vk_mptr, 0x0180)) }
                    lt(mptr, mptr_end)
                    { mptr := add(mptr, 0x20) }
            ...
            // Compute quotient evaluation
            {
                ...
                let y := mload(add(theta_mptr, 0x60))
                {
                    ...
                    let computations_ptr, computations_len := soa_layout_metadata(0x03a0, vk_mptr)
                ...
                }
                {
                    ...
                    let permutation_z_evals_ptr := add(vk_mptr, mload(add(vk_mptr, 0x03c0)))
                ...
                }
                {
                    ...
                    let evals_ptr, meta_data := soa_layout_metadata(0x03e0, vk_mptr)
            ...
            {
                mstore(0x00, calldataload(mload(add(vk_mptr, 0x0360))))
                mstore(0x20, calldataload(add(mload(add(vk_mptr, 0x0360)), 0x20)))
                let x_n := mload(add(theta_mptr, 0x180))
                for
                    {
                        let cptr := sub(mload(add(vk_mptr, 0x0360)), 0x40)
                        let cptr_end := sub(mload(add(vk_mptr, 0x0380)), 0x40)
                    }
                    lt(cptr_end, cptr)
                    {}
                {
            ...
            {
                // point_computations 
                let pcs_ptr := add(vk_mptr, mload(add(vk_mptr, 0x0400)))
                {
                    let point_computations := mload(pcs_ptr)
                    let x := mload(add(theta_mptr, 0x80))
                    let omega := mload(add(vk_mptr, 0x0140))
                    let omega_inv := mload(add(vk_mptr, 0x0160))
                ...
                let nu := mload(add(theta_mptr, 0xC0))
                {
                    ...
                    mstore(0x80, mload(add(vk_mptr, 0x0220)))
                    mstore(0xa0, mload(add(vk_mptr, 0x0240)))
