...
async function main() {
  console.log("🚀 Starting Flow EVM Testnet Deployment");
  console.log("=======================================");
  ...
  if (network !== "flow_testnet") {
    console.error("❌ This script is designed for Flow EVM testnet only!");
    console.log("💡 Use: npx hardhat run scripts/deploy_flow_testnet.js --network flow_testnet");
    process.exit(1);
  }
  ...
  // Check minimum balance (0.01 FLOW recommended)
  const minBalance = hre.ethers.parseEther("0.01");
  if (balance < minBalance) {
    console.error("❌ Insufficient balance! Need at least 0.01 FLOW for deployment.");
    console.log("💡 Get Flow testnet tokens from: https://testnet-faucet.onflow.org/");
    process.exit(1);
  }
  ...
  if (successfulDeployments === contracts.length) {
    ...
    Object.entries(deploymentResults.contracts).forEach(([name, data]) => {
      if (!data.failed) {
        console.log(`${name}: https://evm-testnet.flowscan.io/address/${data.address}`);
      }
    });
  ...
  const resultsFile = `flow-testnet-deployment-${Date.now()}.json`;
  ...
  if (Object.keys(successfulContracts).length > 0) {
    fs.writeFileSync('config/addresses/flow-testnet-addresses.json', JSON.stringify({
      network: "flow_testnet",
      chainId: chainId,
      timestamp: deploymentResults.timestamp,
      contracts: successfulContracts
    }, null, 2));
    console.log("📍 Addresses saved to: config/addresses/flow-testnet-addresses.json");
  }
  
  console.log("\n🎯 Next Steps:");
  console.log("1. View deployed contracts on Flow EVM Explorer");
  console.log("2. Test the deployed contracts with flow_testnet_test.js");
  console.log("3. Add to MetaMask: https://testnet.evm.nodes.onflow.org (Chain ID: 545)");
